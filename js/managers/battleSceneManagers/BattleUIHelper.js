...
export const BattleUIHelper = {
    formatReward(reward, baseStr = "10", effectiveBase = 10, skill = null) {
        if (!reward) return "Нет данных";
        if (typeof reward === 'string') return this.translateEffect(reward).join(', ');
        ...
        if (reward.damageCoef) {
            ...
            p.push(`Урон: ${skill && (skill.hits > 1 || reward.hits > 1) ? (reward.hits || skill.hits)+'x' : ''}${d} <small style="color:#888;">(База ${baseStr} * ${pct}%)</small>`);
        }
    ...
    translateEffect(effectString, isLucky = false, isUnlucky = false, isCrit = false, isSusceptible = false) {
        ...
        effectString.split(',').forEach(part => {
            ...
            } else if (effectId === 'AGAPINGWOUND') {
                text = `Зияющая рана (${val})`;
            } else if (effectBase) {
            ...
            if (skill.uniqueCondition) {
                blocks.push(`
                    <div class="tt-condition" style="flex:1; padding:3px 6px; background:rgba(255,191,0,0.05); border-left:2px solid #ffbf00; font-size: 11px; line-height: 1.2;">
                        ...
                        <span style="color:#4affab; font-weight:bold;">Награда:</span> <span style="color:#fff;">${this.formatReward(skill.uniqueConditionReward, baseStr, effectiveBase, skill)}</span>
                    </div>
                `);
            }
            if (skill.comboOrMarkImproveable && skill.comboChanges) {
                blocks.push(`
                    <div class="tt-combo" style="flex:1; padding:3px 6px; background:rgba(74,255,171,0.05); border-left:2px solid #4affab; font-size: 11px; line-height: 1.2;">
                        ...
                        <div style="color:#fff;">${this.formatReward(skill.comboChanges, baseStr, effectiveBase, skill)}</div>
                    </div>
                `);
            }
